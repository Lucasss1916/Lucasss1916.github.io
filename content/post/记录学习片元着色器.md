...
//half4:返回RGBA(4通道)
//frag：片元着色器函数
//Varyings i：从顶点着色器插值传下来的数据结构
	通常包含uv,positionCS等
//SV_Target:通知GPU这是最终输出到渲染目标（屏幕/RT）的颜色、
//片元着色器每个像素都会执行一次
//half是16bit浮点，性能更好，精度足够用于颜色
half4 frag (Varyings i) : SV_Target
{
	
    // 把 UV 从 [0,1] 移到 [-0.5, 0.5]，把坐标原点移动到中心
    float2 uv = i.uv - 0.5
    // =========================
    // 1️⃣ length：距离（圆形），计算像素点到中心点的距离
    // =========================
